...
public:
	...
	State get_neighbour(std::mt19937& gen, double temperature) const {
		std::uniform_real_distribution<> dist_modifier(-temperature/100.0,temperature/100.0);
...
double stateAcceptanceProb(State current, State neighbour, double temperature){
	return neighbour.energy < current.energy ? 1.0 : std::exp((current.energy - neighbour.energy)/temperature);
}
...
void simulated_annealing(CoolingSchedule cooling_type, double s_temp, double alpha, int ations){
	...
	std::cout << "SA with initial state: (" << current_state.x << ", " << current_state.y << ") and energy: " << current_state.energy << std::endl;

	for(int i = 0 ; i < ations ; ++i ) {
		State neighbour_state = current_state.get_neighbour(gen, temp);
		double acceptance_prob = stateAcceptanceProb(current_state, neighbour_state, temp);
	...
	std::cout << "Optimal state found at: (" << best_state.x << ", " << best_state.y << ")" << std::endl;
	std::cout << "Minimum energy (function value): " << best_state.energy << std::endl << std::endl;
}
