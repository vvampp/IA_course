...
Cell::Cell()
    : left(true), right(true), top(true), bottom(true), is_visited(false), entry(false),
      exit(false) {
...
Cell::Cell(const Cell &other)
    : left(other.left), right(other.right), top(other.top), bottom(other.bottom),
      is_visited(other.is_visited), entry(other.entry), exit(other.exit),
...
bool Cell::check_if_visited() const { return is_visited; }

void Cell::set_visited() { is_visited = true; }

void Cell::set_direction(int x, int y) { direction = std::make_pair(x, y); }

std::pair<int, int> Cell::get_direction() { return direction; }

bool Cell::is_entry() { return entry; }

bool Cell::is_exit() { return exit; }

void Cell::set_as_entry() { entry = true; }

void Cell::set_as_exit() { exit = true; }
