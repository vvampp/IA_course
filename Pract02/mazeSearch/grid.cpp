...
void Grid::set_cell_as_visited(int x, int y) { get_cell(x, y).set_visited(); }

void Grid::set_highlighted_cell(int x, int y) { highlighted_cell = std::make_pair(x, y); }
...
bool Grid::prim_maze_animation_step(std::vector<std::tuple<int, int, int, int>> &walls) {
    ...
    if (cells[x2][y2].check_if_visited() != cells[x1][y1].check_if_visited()) {
        ...
        if (x2 > 0 && !cells[x2 - 1][y2].check_if_visited())
            walls.push_back(std::make_tuple(x2, y2, x2 - 1, y2));
        if (x2 < GRID_HEIGHT - 1 && !cells[x2 + 1][y2].check_if_visited())
            walls.push_back(std::make_tuple(x2, y2, x2 + 1, y2));
        if (y2 > 0 && !cells[x2][y2 - 1].check_if_visited())
            walls.push_back(std::make_tuple(x2, y2, x2, y2 - 1));
    ...
    for (int i = 0; i < GRID_HEIGHT; ++i) {
        for (int j = 0; j < GRID_WIDTH; ++j) {
            if (i == highlighted_cell.first && j == highlighted_cell.second)
                cells[i][j].draw(
                    window, sf::Vector2f(pos_x + SQUARE_SIZE * j, pos_y + SQUARE_SIZE * i), true);
            else
...
void Grid::draw_path(sf::RenderWindow &window) {
    ...
    while (current.first != 0 || current.second != 0) {
        ...
        path_rect.setPosition(pos_x + SQUARE_SIZE * current.second,
                              pos_y + SQUARE_SIZE * current.first);
