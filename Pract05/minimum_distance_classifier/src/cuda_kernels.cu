...
std::string get_cuda_device_info() {
    ...
    for (int i = 0; i < device_count; ++i) {
        ...
        info += "  Total Memory: " + std::to_string(prop.totalGlobalMem / (1024 * 1024)) + " MB\n";
        info += "  Multiprocessors: " + std::to_string(prop.multiProcessorCount) + "\n";
        info += "  Max Threads per Block: " + std::to_string(prop.maxThreadsPerBlock) + "\n";
    }
...
void cuda_classify(const float *h_samples, const float *d_centroids, int *h_predictions,
    ...
    if (centroids_size <= MAX_SHARED_MEMORY_BYTES && n_classes <= MAX_CLASSES_SHARED) {
        ...
        compute_distances_kernel_shared<<<config.grid_dim, config.block_dim,
                                          config.shared_memory_size>>>(
            d_samples, d_centroids, d_distances, n_samples, n_features, n_classes);
        ...
        classify_fused_kernel<<<blocks, threads_per_block>>>(d_samples, d_centroids, d_predictions,
...
void cuda_classify_streams(const float *h_samples, const float *d_centroids, int *h_predictions,
                           int n_samples, int n_features, int n_classes, int n_streams) {
    ...
    std::copy(h_predictions_pinned, h_predictions_pinned + n_samples, h_predictions);
