...
MinimumDistanceClassifier::MinimumDistanceClassifier(bool use_cuda)
    : n_classes_(0), n_features_(0), is_fitted_(false), use_cuda_(use_cuda), d_centroids_(nullptr),
      centroids_size_(0) {
#ifdef USE_CUDA
    if (use_cuda_) {
...
// destructor
MinimumDistanceClassifier::~MinimumDistanceClassifier() { free_cuda_memory(); }

// movement constructors
MinimumDistanceClassifier::MinimumDistanceClassifier(MinimumDistanceClassifier &&other) noexcept
    : centroids_(std::move(other.centroids_)), n_classes_(other.n_classes_),
      n_features_(other.n_features_), is_fitted_(other.is_fitted_), use_cuda_(other.use_cuda_),
      cuda_available_(other.cuda_available_), d_centroids_(other.d_centroids_),
...
// public methods
void MinimumDistanceClassifier::fit(const std::vector<std::vector<float>> &X,
...
std::vector<int>
MinimumDistanceClassifier::predict(const std::vector<std::vector<float>> &X) const {
    if (!is_fitted_) {
...
std::vector<int>
MinimumDistanceClassifier::predict_batch(const std::vector<std::vector<float>> &X) const {
    return predict(X);
}
...
// private methods
void MinimumDistanceClassifier::validate_data(const std::vector<std::vector<float>> &X,
                                              const std::vector<int> &y, bool check_labels) const {
    if (X.empty()) {
    ...
    for (size_t i = 1; i < X.size(); ++i) {
        if (X[i].size() != n_features) {
            ...
            oss << "Inconsistent amount of features. Sample 0 has " << n_features
                << " features, but sample " << i << " has " << X[i].size() << " features.";
    ...
    if (is_fitted_ && static_cast<int>(n_features) != n_features_) {
        std::ostringstream oss;
        oss << "X has " << n_features << " features, but model was trained with " << n_features_
            << " features.";
        throw std::invalid_argument(oss.str());
    }
...
void MinimumDistanceClassifier::compute_centroids(const std::vector<std::vector<float>> &X,
                                                  const std::vector<int> &y) {
    // sum accumulator vector
    std::vector<std::vector<float>> sums(n_classes_, std::vector<float>(n_features_, 0.0f));
...
float MinimumDistanceClassifier::euclidean_distance_squared(
    const std::vector<float> &sample, const std::vector<float> &centroid) const {
    float dist_sq = 0.0f;
...
std::vector<int>
MinimumDistanceClassifier::predict_cpu(const std::vector<std::vector<float>> &X) const {
...
std::vector<int>
MinimumDistanceClassifier::predict_cuda(const std::vector<std::vector<float>> &X) const {
#ifdef USE_CUDA
    // X to contigous
    ...
    cuda_classify(X_flat.data(), d_centroids_, predictions.data(), n_samples, n_features_,
                  n_classes_);
